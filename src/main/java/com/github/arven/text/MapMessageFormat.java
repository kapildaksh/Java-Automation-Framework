...
public class MapMessageFormat extends Format {
     ...
     * @return 
     */
    public Map parse(String source, ParsePosition pos) {
        if (source == null) {
            return Collections.EMPTY_MAP;
        }

        int maximumArgumentNumber = maxOffset;
        Map resultArray = new HashMap();
            ...
            if (formats[i] == null) {   // string format
                ...
                if (next < 0) {
                    ...
                    if (!strValue.equals("{"+argumentNames[i]+"}"))
                        resultArray.put(argumentNames[i], source.substring(sourceOffset,next));
                    sourceOffset = next;
                }
            } else {
                tempStatus.setIndex(sourceOffset);
                resultArray.put(argumentNames[i], formats[i].parseObject(source,tempStatus));
    ...
    @Override
    public Object clone() {
        MapMessageFormat other = (MapMessageFormat) super.clone();
    ...
    public static class Field extends Format.Field {
        ...
        protected Object readResolve() throws InvalidObjectException {
            if (this.getClass() != MapMessageFormat.Field.class) {
    ...
    private static void copyAndFixQuotes(
