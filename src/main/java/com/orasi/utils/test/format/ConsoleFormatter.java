...
public class ConsoleFormatter implements Reporter, Formatter {
    ...
    private boolean featureNeedsSpace = false;
    ...
    public ConsoleFormatter(Appendable out, boolean monochrome, boolean executing) {
        this.out = new NiceAppendable(out);
        this.executing = executing;
        setMonochrome(monochrome);
    }
    
    public ConsoleFormatter(Appendable out) {
        this(out, false, true);
    }

    public final void setMonochrome(boolean monochrome) {
        if (monochrome) {
    ...
    public void feature(Feature feature) {
        if(featureNeedsSpace) {
            out.println();
        }
        if(!featureNeedsSpace) {
            featureNeedsSpace = true;
        }
        printComments(feature.getComments(), "");
        printTags(feature.getTags(), "");
        out.println(AnsiEscapes.INTENSITY_BOLD.toString() + AnsiEscapes.RED.toString() + feature.getKeyword() + ": " + AnsiEscapes.BLUE.toString() + feature.getName() + AnsiEscapes.RESET.toString());
    ...
    private void printStatement() {
        ...
        buffer.append(AnsiEscapes.CYAN).append(AnsiEscapes.INTENSITY_BOLD);
        buffer.append(statement.getKeyword());
        buffer.append(": ");
        buffer.append(statement.getName());
        buffer.append(AnsiEscapes.RESET);
    ...
    public void examples(Examples examples) {
        ...
        out.append(AnsiEscapes.CYAN.toString());
        out.append(AnsiEscapes.INTENSITY_BOLD.toString());
        out.println("    " + examples.getKeyword() + ": " + examples.getName());
        out.append(AnsiEscapes.RESET.toString());
    ...
    private void prepareTable(List<? extends Row> rows) {
        ...
        for (int rowidx = 0; rowidx < rows.size(); rowidx++) {
            ...
            for (int colIndex = 0; colIndex < columnCount; colIndex++) {
                ...
                cellLengths[rowidx][colIndex] = length;
    ...
    public void row(List<CellResult> cellResults) {
        ...
        buffer.append(AnsiEscapes.YELLOW);
        buffer.append(AnsiEscapes.INTENSITY_BOLD);        
        ...
        for (int colIndex = 0; colIndex < maxLengths.length; colIndex++) {
            ...
            switch (row.getDiffType()) {
                case NONE:
                    status = cellResults.size() < colIndex ? cellResults.get(colIndex).getStatus() : "undefined";
                    break;
                case DELETE:
                    status = "skipped";
                    break;
            ...
            if(row.getDiffType().equals(Row.DiffType.NONE)) {
                if(rowIndex < 1)
                    buffer.append(AnsiEscapes.INTENSITY_BOLD);
                buffer.append(AnsiEscapes.YELLOW).append(cellText).append(AnsiEscapes.RESET);
            } else {
    ...
    private void calculateLocationIndentations() {
        ...
        int maxLineWidth = 0;
        for (BasicStatement stmt : statements) {
            int stepWidth = stmt.getKeyword().length() + stmt.getName().length();
