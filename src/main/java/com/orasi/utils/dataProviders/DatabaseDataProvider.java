...
public class DatabaseDataProvider {
    ...
    private static String host = "GSOD-JPHELGAR";
    private static String port = "3306";
    private static String database = "justin";
	...
	String metaSql = "SELECT MAX(Scenario_Num) NUMROWS,  MAX(Data_Field_Order) NUMCOLUMNS  FROM justin.testdata WHERE TEST_NAME= '" + testName + "'";
	String sql = "SELECT DATA_FIELD_NAME, DATA_FIELD_VALUE FROM justin.testdata WHERE TEST_NAME= '" + testName + "' ORDER BY SCENARIO_NUM, DATA_FIELD_ORDER";
	...
	switch(databaseType){
		case "mysql": db = new MySQLDatabase(host, port, database);
				break;
		case "oracle": db = new OracleDatabase(host);
				break;
	}
	...
	Object[][] oldData = db.getResultSet(sql);
	metaRs.moveLast();
	int numberRows = Integer.parseInt(metaRs.getValue("NUMROWS")); 
