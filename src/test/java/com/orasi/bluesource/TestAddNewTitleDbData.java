...
public class TestAddNewTitleDbData {
	...
	public Object[][] scenarios() {
		return new DatabaseDataProvider(DatabaseDataProvider.MYSQL)
				.getTestData("TestAddNewTitle");
	}
	...
	/*
	 * @AfterMethod(groups = { "regression" }) public synchronized void
	 * closeSession(ITestResult test){
	 * System.out.println(test.getMethod().getMethodName()); WebDriver driver =
	 * drivers.get(test.getMethod().getMethodName());
	 * 
	 * //if is a failure, then take a screenshot if (test.getStatus() ==
	 * ITestResult.FAILURE){ new Screenshot().takeScreenShot(test, driver); }
	 * driver.quit(); }
	 */
